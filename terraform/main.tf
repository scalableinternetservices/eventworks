...
resource "aws_ecs_cluster" "eventworks" {
  name = "eventworks"
}

resource "aws_ecr_repository" "eventworks" {
  name                 = "eventworks"
...
module "webserver" {
  ...
  ecr_repository = aws_ecr_repository.eventworks.repository_url
  ecs_cluster    = aws_ecs_cluster.eventworks.id
